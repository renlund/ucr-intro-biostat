...
%%   In an \emph{experiment} the researcher will have control on some of the
%%   variables, typically through randomization. E.g.\ assigning individuals to
%%   different treatment arms in a clinical trial.

%%   \rymd
%%   In an observational study the data no such control exists.
...
\frame{ % -------------------------------------------------------------------->
  ...
  A data set contains one or more \emph{variable} for each unit of study
...
\begin{itemize}
   ...
   \item Numerical
      ...
      \end{itemize}
\end{itemize}
The category of the \emph{outcome variable} determines what analyses are available.
...
\begin{itemize}
   ...
   \item how are missing values represented? "" (blank), ".", "-99"?
\end{itemize}
...
infarctions with a model that includes sex, age, BMI (some missing) and smoking
status (some missing). Worry: the reason for being missing depends on the value.

\rymd The statistical software default is to include only those individuals with
complete data on all variables in the analysis.
...
Suppose you want to model some outcome against 10 covariates. If an individual
has 1 value missing, they will be excluded from the complete case analysis
\emph{event though they could contribute with 9 other covariate values}.

\rymd \textbf{Imputation methods} tries to fill in the blanks; e.g. with
typical values (e.g. mean or sampled values) or predicted values (from
regression models, possibly using more covariates).

\rymd It might even to this several times to create multiple data sets over which
the analysis will be averaged.
...
"It is not that (multiple) imputation is so good; it is really that other methods
  for addressing missing data are so bad."
\begin{flushright}
(Donald Rubin)
\end{flushright}
\end{quote}
...
%% This interesting and complicated topic is outside the scope of this course.
...
A biomarker - the protein S100B - was measured for 113 individuals with
...
\item The $k$th percentile is a value $v$ such that $k$ percent of your data lies below (or at) $v$. (Usually not uniquely defined.)
  ...
  \rymd \item The 50th percentile (the \emph{median}) is the point which divides your ordered sample equally. (Only 'unique' if sample is odd, else use mean of the two midpoints.)
   ...
   \rymd \item We can describe all percentiles with the \emph{cumulative
     frequency graph} (CF) also called the \emph{empirical cumulative
...
step-wise function with jumps according to:
  ...
  \begin{center}\begin{tabular}{|r|r|r|r|} \hline
      Values & Count & Cumulative count & Cumulative proportion \\ \hline
...
\frame{ % -------------------------------------------------------------------->
\frametitle{Cumulative frequency for S100B}
$\phantom X$
...
\begin{itemize}
   ...
   $ \bar x=\frac 1n\sum_{i=1}^n x_i$ is the mean.

   \rymd It is the typical distance between a value and the mean value.
...
Probability theory studies models of random data. A \textbf{model} is a way of
specifying the range of possible values and the probability with which these
occur.
\begin{itemize}
   \item \textbf{Probability functions} describe discrete numeric/categorical
     data
...
A yet undetermined random value is called a \emph{random variable}.
...
at random is a random variable with a probability function
...
par(.par_reset)
@
The Law of Large Numbers tells us that the sample distribution will
increasingly look like the population distribution.
}
\frame{ % -------------------------------------------------------------------->
...
Absolute frequencies can be hard to compare if groups are unbalanced - use
subgroup percentages instead.
<<smoker, fig.height = 6>>=
par(mfrow = c(2,1), mar = c(2,4,1,0))
barplot(tabsmok, beside=TRUE, legend=TRUE, ylab="Frequency",
        col=brewer.pal(9, "YlGn"), xaxt = 'n', args.legend = list(horiz = TRUE))
mtext("Frequency plot", side = 3, line = 0)
barplot(tabsmok3, beside=TRUE, legend=FALSE, ylab="Percent",
        col=brewer.pal(9, "YlGn"))
mtext("Within subgroup percentages", side = 3, line = 0)
par(.par_reset)
@
}
\frame{ % -------------------------------------------------------------------->
\frametitle{Flipping the axes}
...
curve(dunif(x, 0, 1), from = 0, to = 1, xlim = c(-0.2, 1.2), ylim = c(0,1),
      ylab = "Density", xlab = "", bty = "l", lwd = 3)
zero <- function(x) rep(0, length(x))
curve(zero(x), from = -1, to = 0, add = TRUE, lwd = 3)
curve(zero(x), from = 1, to = 2, add = TRUE, lwd = 3)
par(.par_reset)
@
...
\rymd (\textbf{Beware!} In many models/tests it is \emph{not} the data in front
of you which is assumed normal. More on that later in the course.)
...
\rymd \textbf{Note:} Any sample can be "standardized" by subtracting the mean
...
\frame{ % --------------------------------------------------------------------->
\frametitle{Digression on standardization:}
If $x_1, x_2, \ldots, x_n$ is a sample (of size $n$) with mean
$\bar x = \frac1n\sum_1^n x_i$ and
$sd = \sqrt{\frac1{n-1}\sum_1^n(x_i-\bar x)^2}$, then the transformation
\[ \frac{x_1-\bar x}{sd}, \frac{x_2-\bar x}{sd}, \ldots, \frac{x_n-\bar x}{sd}, \]
...
\rymd Here is the albumin measurements again; orginal and standardized:
<<fig.height = 3>>=
load("data/F3_albumin.Rdata")
par(mar=c(4,1,4,1))
plot(
   x = alb, y = rep(1,length(alb)),
   yaxt='n', xaxt = 'n',
   ylab="", xlab = "",
   ylim=c(0.8,1.2),
   bty = 'n',
...
points(sort(alb), rep(1,length(alb)))
xx <- seq(1.7, 2.2, by = 0.1)
m <- mean(alb)
s <- sd(alb)
axis(3, at= xx, labels=TRUE)
mtext("Measurement scale (g / dL)", side = 3, line = 2.5)
axis(1, at= c(m-2*s,m-s, m, m+s,m+2*s), labels=c(-2,-1, 0, 1,2))
mtext("Standardized scale", side = 1, line = 2.5)
par(.par_reset)
@
}
\frame{ % --------------------------------------------------------------------->
...
<<normal_stuff, fig.width=9, fig.height=3.5, include=TRUE>>=
par(mar = c(2, 4, 0,0))
...
par(.par_reset)
@
This will be useful when creating \emph{confidence intervals}.
}
\frame{ % --------------------------------------------------------------------->
...
\rymd
{\centering\fbox{If $Y$ is N$(\mu,\sigma)$ then $(Y-\mu)/\sigma$ is N$(0, 1)$.}\par}

\rymd
{\centering\fbox{If $X$ is N(0,1) then $\mu + \sigma X$ is N$(\mu,\sigma)$.}\par}
<<normals, fig.height = 3.5>>=
par(mar = c(2,4,0,0))
n <- 100
...
In fact, if $X_1$ and $X_2$ are Normal (and \emph{independent}), then so is
\[ a + bX_1 + cX_2. \]
}
\frame{ % --------------------------------------------------------------------->
...
\frametitle{Estimators}
  Suppose you're trying to estimate $\mu$ the average height of a population and you
have a random sample of the heights of 10 individuals available;
$x_1, x_2, \ldots, x_{10}$.

\rymd
Consider the following strategies for estimating $\mu$:

\begin{itemize}
\item A: By the largest value; $\max x_i$
\item B: By the first value; $x_1$
\item C: By the mean value; $\bar x = \frac{1}{10}\sum x_i$
\end{itemize}

\rymd
Two leading questions:
\begin{itemize}
\item What's bad about A?
\item Why is C better than B?
\end{itemize}
...
An estimator is unbiased if it is right on average (as a strategy / as a random
variable), i.e. there is no systematic error. Strategy A is biased.

\rymd
Strategies B and C are both unbiased, but C has a smaller standard deviation (varies
less). Why? Intuitively, some of the $x_i$'s are going to be "large" and some
are going to be "small", and the average value will cancel some of these.

\rymd
For an unbiased estimator, the standard deviation (called the \emph{standard
  error}) is the "typical" error, i.e. a measure of \emph{precision}.

\rymd
If we also knew the distribution of the estimator, we could calculate things
like confidence intervals.

\rymd
Next up: \emph{How does the distribution of the mean value depend on the
  distribution of the population?}

}

\frame{ % -------------------------------------------------------------------->
\frametitle{Estimator distribution: 2 special cases}

Sometimes our model assumptions gives us the answer.

\rymd
\textbf{1.} If the population is normally distributed $N(\mu, \sigma)$

The mean value is a linear combination and so \emph{by theory} (details skipped)
the mean value has the distribution:

$$N\left(\mu, \frac{\sigma}{\sqrt n}\right),$$

where $n$ is the sample size.

%% I.e. the SE for the mean is $\sigma/\sqrt{10}$.

\rymd
\textbf{2.} If the population has a gamma-distribution (skewed).

Details are unimportant, the point is that the mean value distribution
\emph{can} be calculated (look 2 slides ahead).

}


\frame{ % -------------------------------------------------------------------->
\frametitle{Sample mean distribution (normal population)}
...
\frame{ % -------------------------------------------------------------------->
\frametitle{Sample mean distribution (gamma population)}
The distribution of sample means from a skewed gamma distribution. The sample mean
distribution becomes increasingly symmetric.
<<exp_mean>>=
COL <- wes_palette("Cavalcanti1", 5) #brewer.pal(5, "Set2")
par(mar=c(4,4,1,1))
plot(1,1,type='n', xlim=c(0,2.5), ylim=c(0,1.8), xlab="Outcome", ylab="Density")
...
\rymd This is great news! This means we (often) only need to calculate the standard
error.

}

\frame{ % -------------------------------------------------------------------->
\frametitle{Sample mean distribution of coin flips}
Even non-continuous (discrete) distributions can be approximated with the normal
distribution. (Normal density curve rescaled by $1/n$.)
<<>>=
par(mar=c(4,4,1,1))
...
mh <- function(n){
    data.frame(x = 0:n,
               m = 0:n / n,
               p = dbinom(x = 0:n, size = n, prob = 0.5),
               n = n)
}
nset <- c(1,2,5,10,20, 50)
R <- NULL
for(n in nset) R <- rbind(R, mh(n))

xx <- seq(0,1,len = 101)
N <- NULL
for(n in nset){
    d <- dnorm(xx, mean = .5, sd = sqrt(1/(4*n)))
    tmp <- data.frame(n = n, m = xx, N = d, N2 = d/n)
    N <- rbind(N, tmp)
}

ggplot(R, aes(m, p)) +
    geom_bar(width = 0.01, stat = "identity") +
    geom_point() +
    geom_line(data = N, aes(m, N2), color = "red", lty = 2) +
    labs(y = "Probability", x = "Value for mean") +
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Misc. Topics]{Miscellaneous: visual tests, etc} %%%%%%%%%%%%%%%%%%%%%%
\subsection{} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\frame{ % -------------------------------------------------------------------->
\frametitle{Visual tests of normality: Cross-over data}
13 patients had their peak expiratory flow (PEF, l/min) recorded
after inhaling each of two different asthma drugs
...
\frame{ % -------------------------------------------------------------------->
\frametitle{PEV}
One could ascertain the plausibility of an underlying normal distribution via
e.g.\ a histogram or a CF.
...
curve( dnorm(t, mean(y-x), sd(y-x)), from=-60, to=160, ylim=c(0,.016),
      lty=1, xname="t", xlab="Differences (formoterol-sabutamol) L/min",
      main="", ylab = "Density", xlim = c(-50, 150))
hist(y-x, freq=FALSE, breaks=seq(-50,150,len=10),
     xlim=c(-60,160), ylim=c(0,.018),
     col=.blue, add = TRUE)
curve(dnorm(t, mean(y-x), sd(y-x)), from=-50, to=150, lty=2,
      add=TRUE, xname="t")

plot(ecdf(y-x), verticals=TRUE, do.points=FALSE,
     xlab="Differences (formoterol-sabutamol) L/min", ylab="",
     xlim=c(-100,200), main="")
curve( pnorm(t, mean(y-x), sd(y-x)), from=-250, to=200,
      lty=2, add=TRUE, xname="t")
@
}
...
"increasingly standard normal" as its parameter (degrees of freedom) increases.
...
%% \item Brown University \emph{Seeing Theory}  \href{https://seeing-theory.brown.edu/probability-distributions}
%%     {https://seeing-theory.brown.edu/probability-distributions}
%% \end{itemize}
%% }
...
\frame{ % -------------------------------------------------------------------->
\frametitle{Poll: Variables and driving}
\textbf{Q1:} Tick the boxes you think are always true. (Multiple choice)
\begin{itemize}
...
\item[\lada] None of the above.
\end{itemize}

\textbf{Q2:} A poll showed that 80\% of drivers thought their driving skills
were above average. As a group, do these drivers overestimate their driving
skills? (Single choice)
\begin{itemize}
\item[\lada] No
\item[\lada] Yes
\item[\lada] Probably
\item[\lada] We can't say
\end{itemize}
}
\frame{ % -------------------------------------------------------------------->
\frametitle{Poll: Boxplots and means}
\textbf{Q1:} A group of 24 individuals have been randomly assigned to 6
different exercise groups, and their blood pressure is measured after
training. The researchers do a boxplot for the measurements for each group. Is
this a good approach? (Single choice)
\begin{itemize}
...
\end{itemize}
\textbf{Q1:} Suppose the mean athletic skill is somewhat better in group
A as compared to group B (of equal size). If a sport team is fairly admitting members by
assessing athletic skill, we should expect more people from A being admitted. (Single choice)
\begin{itemize}
\item[\lada] True
\item[\lada] False
\item[\lada] We can't say
...
\frame{ % -------------------------------------------------------------------->
\frametitle{Poll: LLN and CLT}
\textbf{Q1:} Suppose you have flipped a (fair) coin 3 times, every time showing H
('Heads'). The Law of Large Numbers tells us that the relative frequency of H
will tend to 50\% in the long run. Does this imply that we now have an increased
probability of T ('Tails')? (Single choice)
\begin{itemize}
\item[\lada] Yes
\item[\lada] No
\end{itemize}
\textbf{Q2:} The Central Limit Theorem says that, althought the distribution of
any numerical characteristic of individuals (income, blood pressure, etc) might
be skewed in small populations, it will be (approx.) normally distributed in
"large enough" populations. (Single choice)
\begin{itemize}
\item[\lada] True
\item[\lada] False
\end{itemize}
