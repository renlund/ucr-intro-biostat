...
<<plot_alb, fig.align='center', fig.height=5, fig.width=11>>=
load("data/F3_albumin.Rdata")
par(mar=c(4,9,0,0))
plot(
   x = alb, y = rep(2,8),
...
points(sort(alb), c(1.1,0.9, rep(1,6)))
...
barchart(a_table, col = wes_palette("Cavalcanti", 5))
pie(a_table, col = wes_palette("Cavalcanti", 5))
@
...
<<exp_mean>>=
COL <- wes_palette("Cavalcanti", 5) #brewer.pal(5, "Set2")
par(mar=c(4,4,1,1))
