...
\begin{itemize}
   ...
   \begin{itemize}
      \item specific results: how likely are deviations from what we expect
...
#            density = 9, ylim = c(0,0.7), lwd = 2, angle = +45, main = paste0("Sample size ", n))
   barplot(prop.table(table(g, s),1), col = wes_palette("Cavalcanti", 5),
...
barplot(pf, width = m, border = "black", col = "black", legend = l2, ylim = c(0,0.7),
        main = "True distribution", density = 15, lwd = 2, angle = +45)
foo_bar(25)
foo_bar(100)
foo_bar(1000, l = TRUE)
layout(matrix(1, nrow = 1))
