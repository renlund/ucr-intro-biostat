...
\begin{itemize}
   ...
   \item Categorical and
      \begin{itemize}
         ...
         \item ordinal, e.g.\ "Happiness" is ordered: $\frown, -, \smile$.
      \end{itemize}
\end{itemize}
}
\frame{ % -------------------------------------------------------------------->
...
\rymd These may contain "the" average and "the" spread (continuous variables) or counts of levels (for categorical variables)
...
\frame{ % -------------------------------------------------------------------->
\begin{table}\begin{center}
\caption{Data set on 42 individuals.}
\begin{tabular}{|cccc|} \hline
...
\frame{ % -------------------------------------------------------------------->
Example: Table 1 from "The effect of obesity on antibiotic treatment failure: a historical cohort study", \emph{Pharmacoepidemiol.\ Drug. Saf.} (2013)
\begin{center}
\includegraphics[scale=.65]{pics/table1_ex3.png}
\end{center}
}
...
Describe the missing data separately.
...
\frame{ % -------------------------------------------------------------------->
\frametitle{Dotplot of fibrinogen data}
...
\begin{itemize}
\item  \textbf{The mode:} is the single most common data point. \\
...
Some points:
\begin{itemize}
   \item Small data sets does not really need summary measures.
...
(0\% and 100\%.)\\
   ...
   \rymd \item The basic rule of probability is that if $A$ and $B$ are
...
\rymd Some are easy to model realistically, let e.g.\ $Z=$'the outcome of the throw of a die'.
Then $\pr(Z=k)=1/6$ for all $k=1,2,\ldots,6$, or, equivalently

\begin{center}\begin{tabular}{|c|cccccc|} \hline
Value $k$ & 1  & 2 & 3 & 4 & 5 & 6 \\
$\pr(X=v)$ & 1/6 & 1/6 & 1/6 & 1/6 & 1/6 & 1/6\\ \hline
\end{tabular}\end{center}

\rymd Others are difficult. Let $Y=$'the number of cigarettes a person selected at random smoked yesterday'.
...
<<plot_smoker2>>=
load("data/F8_smoking.Rdata")
with(dd, tabsmok3 <<- prop.table(table( age=age, smoker=smoker), 2))
par(mar=c(4,4,0,0))
...
Assume that height of individuals in some population is Normally distributed with
...
\rymd We want to know the population mean ($\mu$).

\rymd We can estimate with the sample mean ($\mu^*$).
...
for(k in 1:m){
   ...
   points(x=tmp, y=rep(m+1-k,n), pch=k, col=col)
   points(x=mtm, y=m+1-k, cex=2, col=col, pch=k, lwd=3)
   points(x=mtm, y=0, cex=2, col=col, pch=k, lwd=3)
