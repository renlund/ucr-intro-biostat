...
\begin{itemize}
   ...
   \item numerical values stored as text (due to ',/.")
\end{itemize}

\rymd
Usually (bias alert!) one only needs to distinguish
\begin{itemize}
   \item measurements (\code{Age}, \code{Albumin}) are numerical
...
\rymd \emph{Complete case analysis} will only give an unbiased result if the reason that a value is missing has nothing to do with the actual value.
...
\rymd \textbf{Single imputation:} replace missing value with a "typical" value for that variable (e.g. mean or median). Better: try to model the variable and predict it from the other variables. Both methods underestimates the variance in the variable and will give overly optimistic results.

\rymd \textbf{Multiple imputation:} create multiple imputed data sets where the missing values are replaced differently in each, e.g. drawn at random from the non-missing values.  Better: use (random) estimates from modeling the variable.

\rymd
\begin{quote}
"It is not that multiple imputation is so good, it is that all other methods are so bad."
\begin{flushright}
(Donald Rubin)
\end{flushright}
\end{quote}
