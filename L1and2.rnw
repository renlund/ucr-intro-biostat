...
\textbf{Fact:} to calculate e.g.\ a confidence interval for $X$, we need to
know the distribution (at least approximately).

\rymd
We want to estimate a characteristic of a population, we will use \textbf{the
  mean} as an example.

\rymd
We may have no prior information about the population, but we get some
information from a random sample. (Assume sample size $> 1$.)

\rymd
We estimate the population mean by the sample mean.

\rymd
What is the distribution of the sample mean (considered as a random number)?

\rymd
Hard to know - we only ever get 1 such number!

\rymd
("Bootstrap" might be a work around.)
}
\frame{ % -------------------------------------------------------------------->
\frametitle{Mean value distribution $\neq$ population/sample distribution}

Consider the simple case of the mean of 2 dice.

<<"twice-dice", include = TRUE, fig.height = 6>>=
d1 <- data.frame(
    x = 1:6,
    p = rep(1/6, 6),
    w = "Single throw ('Population')"
)
tmp <- cumsum(rep(1/36, 6))
d2 <- data.frame(
    x = c(2:12) / 2,
    p = c(tmp, rev(tmp)[2:6]),
    w = "Mean of 2 throws"
...
## table(x)
## x1 <- sample(1:6, size = 100, replace = TRUE)
## x2 <- sample(1:6, size = 100, replace = TRUE)
## mx <- (x1 + x2) / 2
## d1b <- data.frame(
##     x = 1:6,
...
## d2b <- data.frame(
##     x = 2:12 / 2,
##     p = as.numeric(table(mx)),
##     w = "100 mean values (2 throws)"
## )
...
ggplot(d, aes(x, p)) +
    ...
    facet_wrap(~w, ncol = 1, as.table = FALSE)
...
\item What happens to the distribution of the sample mean as the sample size
  increases?
\end{itemize}
...
\begin{itemize}
\item it has a smaller standard devation, and
\item it tends to be more symmetric.
\end{itemize}

\rymd
But again: the sample mean distribution is unobserved, in general we must rely
on theory to determine its properties.
}
%% \frame{ % -------------------------------------------------------------------->
...
%% }
\frame{ % -------------------------------------------------------------------->
  \frametitle{Sampling from a normally distributed population?}

{\centering \fbox{"Normal population" $\Rightarrow$ "Normal sample mean"}\par}

\rymd

\rymd
A random sample of size $n$ from $N(\mu, \sigma)$ has the property that its mean
is also normally around $\mu$ but with
\[ \sigma_{\mbox{\footnotesize sample mean}} = \frac{\sigma}{\sqrt{n}}. \]

\rymd
But, not everything is normally distributed to begin with!
}
\frame{ % -------------------------------------------------------------------->
...
The (estimated) standard deviation of an estimator is called its
\textbf{standard error}.

\rymd
The CLT allows us in many situations to ignore the actual distribution of the
population and the estimator, but we typically need the standard error.

\rymd
Exmple: suppose we want to estimate the height of individuals in a population.
A sample of 50 people gave a mean 175 cm with a sample standard deviation of
6.5. Then $se = 6.5 / \sqrt{50} \approx 0.92.$

\rymd
As will be shown later the standard error help us to
\begin{itemize}
\item create confidence interval, and
\item create \emph{test statistics} for hypothesis testing.
\end{itemize}
}
\frame{ % -------------------------------------------------------------------->
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Misc.]{Miscellaneous: visual tests, missing data} %%%%%%%%%%%%%%%%%%%%
\subsection{} %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% \frame{ % -------------------------------------------------------------------->
%% <<"test-no-eval", eval = FALSE>>=
%% m1 <- 10; m2 <- 10.5
%% foo <- function(x, a = m1, b = m2){
%%     p1 <- 1 - pnorm(x, a, 1)
%%     p2 <- 1 - pnorm(x, b, 1)
%%     p2 / p1
%% }
%% curve(foo(x), 8, 12, ylim = c(0, 7))
%% curve(dnorm(x, m1, 1), 8, 12, add = TRUE, lty = 2)
%% curve(dnorm(x, m2, 1), 8, 12, add = TRUE)
%% @
%% }
