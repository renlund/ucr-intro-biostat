...
\rymd \textbf{Single imputation} e.g.\ replace missing value with a "typical" value for that variable (e.g. mean or median). This method (and similar ones) underestimates the variance in the variable and will give overly optimistic results.

\rymd \textbf{Multiple imputation} create multiple imputed data sets where the missing values are replaced differently in each iteration, e.g. drawn at random from the non-missing values.
