...
\rymd
How does the distribution of the mean value depend on the distribution of the
population?
...
The mean value is a linear combination and so \emph{by theory} the mean value
has a distribution (details skipped)

$$N(\mu, \sigma/\sqrt n),$$

where $n$ is the sample size. I.e. the SE for the mean is
$\sigma/\sqrt{10}\approx\sigma^*/\sqrt{10}$.
...
This is great news! This means we (often) only need to calculate the standard
error.
