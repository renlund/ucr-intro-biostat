...
\begin{itemize}
   \item a model (probability specification), or
...
\rymd We would expect $X$ to be 17-18, but is $X = 27$ within some acceptable range of possiblities?
}
\frame{ % -------------------------------------------------------------------->
...
barplot(table(y)/length(y),  xlab = "Assigned Dabigatran",  main = "", space = 0, xaxt = 'n')
...
\rymd We can apply $\chi^2$-tests to binary data, e.g. the Dabigatran data.
...
the situation. (This is not an exact test.)
}
\frame{ % -------------------------------------------------------------------->
...
\vspace{0.2cm}
One way to deal with this is logistic regression (more on that in Lecture 10).
}
