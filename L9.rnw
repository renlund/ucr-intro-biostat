...
This is an estimate! (The error will depend on both the resample \emph{and} inital sample size.)
...
ggplot(POW1, aes(size, est, min=low, max=high)) +
    ...
    scale_y_continuous(limits=c(0,1.01), breaks=c(seq(0,0.8,0.2), 0.9, 1)) +
    theme_classic() +
