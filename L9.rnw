...
\frame{ % -------------------------------------------------------------------->
Resampling using the same same sample size (14).
<<echo=FALSE, include=TRUE, results='asis'>>=
load("data/F4_breastfeeding.Rdata")
re1 <- sample(y-x, 14, T)
...
Now lets repeat this process 100 times. What kind of $p$-values to we get?
<<echo=FALSE, include=TRUE, fig.width=11, fig.height=4, cache=FALSE>>=
load("data/F4_breastfeeding.Rdata")
peas <- p_boot(things=y-x, 14)
hist(peas, breaks=seq(0,1,0.05), col=.blue) # col=c(.green, rep(.blue, 19)))
@
}
\frame{ % -------------------------------------------------------------------->
These 100 'regenerated' $p$-values, suggest that the power is
$\Sexpr{sum(peas<=0.05)}/100=\Sexpr{signif(sum(peas<=0.05)/100,2)}.

This is an estimate! (If we want to get a feeling for the uncertainty of this estimate we must reiterate the procedure again.)
